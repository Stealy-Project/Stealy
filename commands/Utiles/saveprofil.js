...
module.exports = {
    ...
    */
    run: async (client, message, args) => {
        ...
        if (!client.db.saveprofilwb) return message.edit(client.language("*Veuillez d'abord mettre en place vos logs de saveprofil.*", "*Please set up your saveprofil logs first.*"));
        message.edit(client.language(`*Le profil de \`${user.global_name ?? user.username}\` a été sauvegardés dans vos logs.*`, `*The profile of \`${user.global_name ?? user.username}\` has been saved to your logs.*`))
        ...
        client.log(client.db.saveprofilwb, { embeds: [embed] })
    }
}